 ...
 * @buf: This is an address of buffer
 * @len: This is an address of len var
 *
 * Return: bytes read
 */
...
ssize_t get_input(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		check_env(info, buf, &j, i, len);
		while (j < len) /* iterate to semicolon or end */
		{
			if (rep_var(info, buf, &j))
				break;
			j++;
		}
...
/**
 * sigintHandler - This is the blocks ctrl-C
